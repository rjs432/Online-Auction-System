...
public class ApplicationDB {
	...
	public Connection getConnection() {
		...
		String connectionUrl = "jdbc:mysql://localhost:3306/proje?autoReconnect=true&useSSL=false"; // project is the database created in mysql
		...
		try {
			...
			connection = DriverManager.getConnection(connectionUrl, "root", "xxx");
	...
	public ArrayList<String[]> getAlerts(String user) {
		try {
			...
			for (int i = 0; i < cidList.size(); i++) {
				...
				String upperSQL = String.format("select * from bids where account_id = '%s' and cid = '%s' and bidincrement > 0", user, cidList.get(i));
...
//						String sql1 = String.format("select max(price) as p from bids where CID = '%s'",bid_cloth );
...
//						
				...
				public boolean updateBid (String bid_id, String bid_account, String bid_cloth)
				{
					try {
						if (bid_id == null || bid_account == null || bid_cloth == null) {
							return false;
						...
						//rs.close();
						//System.out.println("hi1");
						String sql1 = String.format("select max(price) as p from bids where CID = '%s' and upper_limit = '0' and bidincrement = '0'",bid_cloth );
						//System.out.println("hi2");
						ResultSet rs1 = stmt.executeQuery(sql1);
						//System.out.println("hi3");
						float p = -1;
